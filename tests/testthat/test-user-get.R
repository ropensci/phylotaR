...
test_that("get_ntaxa() works", {
  ...
  cid <- sample(pssbls, 1)
  res1 <- get_ntaxa(
    phylota = phylota, cid = cid, sid = NULL, rnk = NULL,
    keep_higher = FALSE
  )
  expect_true(ntxs[[cid]] == res1)
  res2 <- get_ntaxa(
    phylota = phylota, cid = cid, sid = NULL, rnk = "family",
    keep_higher = FALSE
  )
  expect_true(res1 >= res2)
  res3 <- get_ntaxa(
    phylota = phylota, cid = cid, sid = NULL,
    rnk = "subspecies", keep_higher = TRUE
  )
  expect_true(res1 >= res3)
  res4 <- get_ntaxa(
    phylota = phylota, cid = NULL, sid = phylota[[cid]]@sids,
