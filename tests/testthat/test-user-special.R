...
test_that("read_phylota() works", {
  ...
  phylotaR:::obj_save(
    wd = ps[["wd"]], nm = "clstrs_sqs",
    obj = list(
      "clstrs" = phylota@clstrs,
      "sqs" = phylota@sqs
    )
  )
...
test_that("write_sqs() works", {
  ...
  sids <- sample(sids, n)
  write_sqs(
    phylota = phylota, outfile = file.path(wd, "test.fasta"),
...
test_that("plot_phylota_treemap() works", {
  ...
  p1 <- plot_phylota_treemap(
    phylota = phylota, cids = cids, area = area,
    fill = fill
  )
  p1
  ...
  p2 <- plot_phylota_treemap(
    phylota = phylota, txids = txids, area = area,
    fill = fill
  )
  p2
...
test_that("plot_phylota_pa() works", {
  ...
  cids <- phylota@cids[1:n]
  spp <- unique(get_txids(
    phylota = phylota, txids = phylota@txids,
    rnk = "species"
  ))
  spp <- spp[spp != ""]
...
test_that("mk_txid_in_sq_mtrx() works", {
  ...
  res <- phylotaR:::mk_txid_in_sq_mtrx(
    phylota = phylota, txids = txids,
    sids = sids
  )
  expect_true(inherits(res, "matrix"))
  sid <- sample(colnames(res), 1)
  txid <- sample(rownames(res), 1)
  ads <- phylotaR:::descendants_get(
