...
#' @examples
#' # TODO
# TODO: maybe need unique filename, maybe add if it is a direct
# or subtree cluster to calculate
make.blast.db <- function(seqs, dbfile='blastdb.fa', dir='.',
    ...
    for (gi in names(seqs)) {
        write(paste0("> ", gi, "\n", seqs[[as.character(gi)]]$seq),
              file=file, append=T)
    }
    cmd <- paste(prmtrs[['mkblstdb']], '-in', file, '-dbtype nucl')
...
#' @examples
#' # TODO
## TODO: maybe need unique filename, maybe add if it is a direct or
# subtree cluster to calculate
blast.all.vs.all <- function(dbname='blastdb.fa', evalue.cutoff=1.0e-10,
                             outfile='blastout.txt', dir='.',
                             prmtrs=prmtrs) {
    ...
    cmd <- paste(prmtrs[['blstn']], '-query', dbname, '-db', dbname, '-outfmt',
                 outfmt, '-dust no -strand plus -evalue', evalue.cutoff,
                 '-out', outfile)
    system(cmd) == 0 || stop('Command ', cmd, 'did not return 0')
    
    ## catch no-result case
    if (! file.exists(outfile)) {
        cat("No BLAST output, returning NULL\n")
        return(NULL)
    }
    ...
    colnames(blast.results) <- c('query.id', 'subject.id', 'identity',
...
filter.blast.results <- function(blast.results, seqs, min.coverage=0.51) {
    ...
    remove.dt <- blast.dt[qcovs/100 < min.coverage,
                          .(query.id, subject.id)][, rbind(.SD, rev(.SD),
                                                           use.names = FALSE)]
