...
getSqsByTxid <- function(txid, phylt_nds, mx_len=25000,
                         mdl_thrshld=10000, verbose=FALSE) {
  .cp(v=verbose, "Retrieving sequences for taxid [", txid, "]")
  ...
  if(subtree_count <= mdl_thrshld) {
    .cp(v=verbose, '[', subtree_count, "] seqs for taxid [",
...
#' @export
fltr <- function(txid, phylt_nds, mdl_thrshld,
  ...
  while(length(queue) > 0) {
    ...
    .cp(v=verbose, "Counting species for taxon [",
        tmp_id, "]")
    ...
    .cp(v=verbose, "Number of sequences for taxon [",
        tmp_id, "]: [", nsqs, "]")
    # if sequence count is smaller than mx_blst_sqs,
    # add it to the phylt_nds to process
    # otherwise look up direct descendents
    if(nsqs <= mx_blst_sqs) {
      .cp(v=verbose, "Will process taxon [", tmp_id, "]")
      res <- c(res, tmp_id)
    } else {
      .cp(v=verbose, "Too many seqs to blast for taxid [",
